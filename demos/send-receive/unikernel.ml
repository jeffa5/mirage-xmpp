...
module Main (S : Mirage_stack_lwt.V4) = struct
  ...
  let rec echo flow =
    ...
    >>= function
    | Ok `Eof ->
      Logs.info (fun f -> f "Closing connection due to Eof!");
      Lwt.return_unit
    | Error e ->
      Logs.warn (fun f ->
          f "Error reading data from established connection: %a" S.TCPV4.pp_error e );
      Lwt.return_unit
    | Ok (`Data b) ->
      Logs.debug (fun f -> f "read: %d bytes:\n%s" (Cstruct.len b) (Cstruct.to_string b));
      let reply = Cstruct.of_string ("echo: " ^ Cstruct.to_string b) in
      write_string flow reply >>= fun () -> echo flow
  ;;
  ...
  let on_connect flow =
    ...
    write_string flow welcome >>= fun () -> echo flow >>= fun () -> S.TCPV4.close flow
  ;;
